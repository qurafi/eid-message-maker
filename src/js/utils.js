function ctxWordWrap(ctx, text, max_width) {
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    let width = Math.abs(mtext.actualBoundingBoxLeft+mtext.actualBoundingBoxRight);
...
function radialGradiant(ctx, x, y, c1, c2) {
  ...
  let r1 = width/6, r2 = height*.75
  let gradiant = ctx.createRadialGradient(x, y, Math.min(r1, r2), x, y, Math.max(width*.7, height*.7));
