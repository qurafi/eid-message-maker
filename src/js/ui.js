...
function initUI() {
  ...
  let span_cb = document.querySelectorAll(`input[type="checkbox"] + span, input[type="radio"] + span`);
  for (const c of span_cb) c.addEventListener("click", toggleCheckboxes);


  ui.sidebar.onscroll = () => ui.nav.classList.toggle("shadow", ui.sidebar.scrollTop >= 5)
...
function initInputs() {
  let inputs = document.querySelectorAll("input:not([type='color']), textarea, select")
