...
function fitToCanvas(width, height) {
  ...
  let ratio = Math.min(
    cwrapper_rect.width / width,
    cwrapper_rect.height / height
  );
...
async function updateSVGColor() {
  for (const id in ui.svgs) {
    ...
    svg
      .querySelector("path")
      .setAttribute("fill", scheme[0][1] + (scheme[1] || "05"));
...
function draw(ctx, bg_aspect, ar) {
  ...
  let [w, h] = aspect(width * 0.7, height * 0.6, 2160, 1080);
  let x1 = (width - w) / 2,
    y1 = (height - h) / 2;
  ...
  ctx.font =
    title_bold +
    " " +
    rfs(scheme[2] && temp != 2 ? 30 : 50, width) +
    "px " +
    font_title;
  ...
  ctx.font =
    message_bold +
    " " +
    rfs(scheme[2] || temp == 2 ? 22 : 30, width) +
    "px " +
    font_message;
  let lines = drawText(
    ctx,
    ...
    x1 + w / 2,
    height * 0.46 + (scheme[2] ? 0 : 15) + logom,
    w -
      (scheme[2] ? (temp == 1 ? 200 : 150) : temp == 2 ? 150 : 0) -
      (message_bold ? 50 : 0)
  );
  ...
  if (ui.imgs.logo) {
    ...
    ctx.font =
      title_bold +
      " " +
      rfs(scheme[2] && temp != 2 ? 25 : 30, width) +
      "px " +
      font_title;
...
export { initCanvas, draw, updateTheme, exportCanvas, updateSVGColor, scheme };
